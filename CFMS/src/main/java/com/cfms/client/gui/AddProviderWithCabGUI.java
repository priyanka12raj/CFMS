 ...
 * a gui to add new provider when a new cab is requested by the user
 */
public class AddProviderWithCabGUI extends Composite {

        public int IS_PROVIDER_ADDED = 0;
        /**
         * a table to hold all labels and text boxes
         */
        FlexTable providerTable;
        /**
         * text boxes for company name, address, email and mobile
         */
        TextBox compname_text, addr, email_text, mobile_no;
        /**
         * a label to display error
         */
         ...
         *
         */
        final VerticalPanel vp2;
        AddNewCabGUI gui;

        public AddProviderWithCabGUI(final Panel contentPanel, final AddNewCabGUI gui) {
                this.vp2 = gui.vp2;
                this.gui = gui;
        ...
        /**
         * @param cabProvider a method to add new cab provider when a new cab is
         *                    requested by the user and the admin is adding a cab with new provider
         */
        private void addNewCabProvider(CabProviderBean cabProvider) {
                InfoService.Util.getService().addCabProvider(cabProvider, new MethodCallback<CabProviderBean>() {
                        ...
                        @Override public void onSuccess(Method method, CabProviderBean response) {
                                if (response != null) {
                                        //				AlertBox.show("provider got added");
                                        vp2.clear();
                                        gui.set_enable();
                                        gui.provider_names_text.setText(response.getCompany_name());
